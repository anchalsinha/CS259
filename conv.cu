...
__global__ void conv_cu(    VTYPE(&synapse)[Ky][Kx][Nn][Ni],
                            VTYPE(&neuron_i)[NYPAD][NXPAD][Ni],
                            VTYPE(&neuron_n)[NYSCL][NXSCL][Nn]
                        )
{
    if(blockIdx.x * blockDim.x + threadIdx.x < (Nx*Ny))
    {
        int ix = (blockIdx.x * blockDim.x + threadIdx.x) % Nx;
        int iy = (blockIdx.x * blockDim.x + threadIdx.x) / Ny;
        
        //loop through output channel
        for(int out=0;out<Nn;out++)
        {
            VTYPE sum = 0;
            //convolve input position +-1 in all directions with kernel
            for(int y=iy;y<iy+3;y++)
            {
                for(int x=ix;x<ix+3;x++)
                {
                    ...
                    {
                        sum += neuron_i[y][x][in] * synapse[y-iy][x-ix][out][in];
                    }
                }
            }
            ...
            if(sum < 0)
            {
                neuron_n[iy][ix][out] = sum/4;
            }
            else
            {
                neuron_n[iy][ix][out] = sum;
            }
...
int main(const int argc, const char** argv) {
    ...
    VTYPE(*synapse_cu)[Ky][Kx][Nn][Ni];
    VTYPE(*neuron_i_cu)[NYPAD][NXPAD][Ni];
    VTYPE(*neuron_n_cu)[NYSCL][NXSCL][Nn];
    ...
    cudaMemcpy(neuron_i_cu, neuron_i, NYPAD*NXPAD*Ni*sizeof(VTYPE), cudaMemcpyHostToDevice);    
    cout << "neuron input copied\n";
    cudaMemcpy(synapse_cu, synapse, SYNAPSE_SIZE*sizeof(VTYPE),cudaMemcpyHostToDevice);
    cout << "synapse copied\n";
    ...
    compare((VTYPE*)*neuron_n,(VTYPE*)*neuron_n_cu,NYSCL*NXSCL*Nn);
