...
void classifier_layer_GPU(VTYPE (&synapse)[Nn][Ni], VTYPE (&neuron_i)[Ni], VTYPE (&neuron_n)[Nn]) {
  int total_calc=0;
  for (int n = 0; n < Nn; n++) {
    VTYPE temp=0;
    for (int i = 0; i < Ni; i++) {
...
__global__ void classifier_layer_GPU(VTYPE (&synapse)[Ni][Nn], VTYPE (&neuron_i)[Ni], VTYPE (&neuron_n)[Nn]) {
  int n = blockIdx.x * blockDim.x + threadIdx.x;
  int index = blockIdx.y * blockDim.y + threadIdx.y;
  VTYPE acc = 0;

  __shared__ VTYPE neuron_i_batch[batch_size];

  if (threadIdx.x < batch_size)
    neuron_i_batch[threadIdx.x] = neuron_i[index + threadIdx.x];

  __syncthreads();

  for (int i = 0; i < batch_size; i++) {
    acc += synapse[index + i][n] * neuron_i_batch[i];
  }
...
// void classifier_layer_GPU(VTYPE (&synapse)[Ni][Nn], VTYPE (&neuron_i)[Ni], VTYPE (&neuron_n)[Nn]) {
